 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'Weblog_Theme_Info' ) ) {
    class Weblog_Theme_Info{
        ...
        public function at_theme_info_welcome_admin_notice() {
            ...
            echo ( '<p>' . sprintf( __('Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.','weblog'), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '" class="button" style="text-decoration: none;">' . sprintf( __('Get started with %s','weblog'), $this->theme_name ) . '</a></p>' );
            echo '</div>';
        }
        ...
        public function prepare_class() {
            ...
            $this->page_title    = isset( $this->config['page_title'] ) ? $this->config['page_title'] : esc_html__('Info','weblog'). $this->theme_name;
            $this->menu_title    = isset( $this->config['menu_title'] ) ? $this->config['menu_title'] : esc_html__('Info','weblog') . $this->theme_name;
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'weblog' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'weblog' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'weblog' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'weblog' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions','weblog').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download','weblog' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview','weblog' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<tr>';
                    echo '<th></th>';
                    echo '<th>' . esc_html__( 'Weblog','weblog' ) . '</th>';
                    echo '<th>' . esc_html__( 'WeblogPro','weblog' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td colspan="2"><a href="https://www.acmethemes.com/themes/weblogpro/" target="_blank" class="button button-primary button-hero"> WeblogPro</a></td>';
                    echo '</tr>';
...
$config = array(
    // Page title.
    'page_title'               => esc_html__( 'Weblog Info', 'weblog' ),

    // Menu name under Appearance.
    'menu_title'               => esc_html__( 'Weblog Info', 'weblog' ),

    // Main welcome title
    'info_title'         => sprintf( esc_html__( 'Welcome to %s - ', 'weblog' ), 'Weblog' ),

    // Main welcome content
    'info_content'       => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help you. If you want to ask any query or just want to say hello, you can always contact us. We hope you enjoy it! ', 'weblog' ), '<b>Weblog</b>' ),
    ...
    'quick_links'                    => array(
        'theme_url'  => array(
                'text' => __('Theme Details','weblog'),
                'url' => 'https://www.acmethemes.com/themes/weblog/'
        ),
        'demo_url'  => array(
            'text' => __('View Demo','weblog'),
            'url' => 'http://www.demo.acmethemes.com/weblog/'
        ),
        'pro_url'  => array(
            'text' => __('View Pro Version','weblog'),
            'url' => 'https://www.acmethemes.com/themes/weblogpro/'
        ),
        'rate_url'  => array(
            'text' => __('Rate This Theme','weblog'),
            'url' => 'https://wordpress.org/support/theme/weblog/reviews/?filter=5'
        ),
    ),

    'tabs'                    => array(
        'getting_started'  => __( 'Getting Started', 'weblog' ),
        'recommended_actions' => __( 'Recommended Actions', 'weblog' ),
        'recommended_plugins' => __( 'Useful Plugins','weblog' ),
        'support'       => __( 'Support', 'weblog' ),
        'changelog'        => __( 'Changelog', 'weblog' ),
        'faq'        => __( 'FAQ', 'weblog' ),
        'free_pro'         => __( 'Free VS PRO', 'weblog' ),
    ),

    /*Getting started tab*/
    'getting_started' => array(
        'first' => array(
            'title' => esc_html__( 'Step 1 : Read full documentation','weblog' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Weblog.','weblog' ),
            'link_title' => esc_html__( 'Documentation','weblog' ),
            'link_url' => 'http://www.doc.acmethemes.com/weblog/',
            'is_button' => false,
            'recommended_actions' => false,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Step 2 : Go to Customizer','weblog' ),
            'desc' => esc_html__( 'All Setting, Theme Options, Widgets and Menus are available via Customize screen. Have a quick look or start customization!','weblog' ),
            'link_title' => esc_html__( 'Go to Customizer','weblog' ),
        ...
        'third' => array(
	        'title' => esc_html__( 'Step 3 : Setting Featured Section ','weblog' ),
	        'desc' => esc_html__( 'Add Feature Image or Unlimited Slides on Feature Section. Please do the Step 3 before this.','weblog' ),
	        'link_title' => esc_html__( 'Featured Section','weblog' ),
	        'link_url' => esc_url( admin_url( 'customize.php' ) ).'?autofocus[panel]=weblog-feature-panel',
	        'is_button' => true,
	        'recommended_actions' => false,
	        'is_new_tab' => true
        )
    ...
    'recommended_actions'        => array(
        'demo-content' => array(
            'title' => esc_html__( 'Install Demo Setup','weblog' ),
            'desc' => sprintf( esc_html__( 'For the demo content installation, install the Acme Demo Setup plugin, download demo content from %1$s here %2$s. Then go to "Acme Demo Setup" and import demo. ', 'weblog' ), '<a target="_blank" href="http://www.doc.acmethemes.com/weblog/#DummyData" >','</a>' ),
            'id' => 'acme-demo-setup',
    ...
    'faq'      => array(
	    'first' => array (
		    'title' => esc_html__( 'Where to get a dedicated support ? ','weblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Follow the theme dedicated support forum.','weblog' ),
		    'button_label' => esc_html__( 'Click here to get the support','weblog' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/supports' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'second' => array (
		    'title' => esc_html__( 'Recent Updates of your Site ? ','weblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Where i can get the recent updates of theme related updates ? ','weblog' ),
		    'button_label' => esc_html__( 'Visit our site blog page','weblog' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/blog/' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'third' => array (
		    'title' => esc_html__( 'How to hide/change footer text Proudly powered by WordPress | Theme: Weblog by AcmeThemes. ? ','weblog' ),
		    'icon' => 'dashicons dashicons-sos',
		    'button_label' => esc_html__( 'Visit this blog post to know how to change or remove powered by text.','weblog' ),
    ...
    'support_content'      => array(
        'first' => array (
            'title' => esc_html__( 'Contact Support','weblog' ),
            'icon' => 'dashicons dashicons-sos',
            'desc' => esc_html__( 'Got theme support question or found bug? Best place to ask your query is our dedicated Support forum.','weblog' ),
            'button_label' => esc_html__( 'Contact Support','weblog' ),
            'button_link' => esc_url( 'https://www.acmethemes.com/supports/forum/weblog/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Documentation','weblog' ),
            'icon' => 'dashicons dashicons-book-alt',
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use Weblog.','weblog' ),
            'button_label' => esc_html__( 'Read full documentation','weblog' ),
            'button_link' => 'http://www.doc.acmethemes.com/weblog/',
            'is_button' => false,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Need more features?','weblog' ),
            'icon' => 'dashicons dashicons-screenoptions',
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.','weblog' ),
            'button_label' => esc_html__( 'View Pro Version','weblog' ),
            'button_link' => 'https://www.acmethemes.com/themes/weblogpro/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Got sales related question?','weblog' ),
            'icon' => 'dashicons dashicons-cart',
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.",'weblog' ),
            'button_label' => esc_html__( 'Pre-sale Question?','weblog' ),
            'button_link' => 'https://www.acmethemes.com/pre-sale-question/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Customization Request','weblog' ),
            'icon' => 'dashicons dashicons-hammer',
            'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.','weblog' ),
            'button_label' => esc_html__( 'Customization Request','weblog' ),
    ...
    'free_pro' => array(
	    array(
		    'title'=> __( 'Custom Widgets', 'weblog' ),
		    'desc' => __( 'All added custom widgets are ready for page builder plugin', 'weblog' ),
		    'free' => __('1+ Available','weblog'),
		    'pro'  => __('6+ Available','weblog'),
	    ),
	    array(
		    'title'       => __( 'Widget Options', 'weblog' ),
		    'desc' => __( 'Every widgets have multiple options to customize section', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Google Fonts', 'weblog' ),
		    'desc' => __( 'Google fonts options for changing the overall site fonts ', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => __('100 +','weblog'),
	    ),
	    array(
		    'title'       => __( 'Color Options', 'weblog' ),
		    'desc' => __( 'Color options for changing overall site colors, Contat us', 'weblog' ),
		    'free'  => __('2','weblog'),
		    'pro'   => __('Unlimited','weblog'),
	    ),
	    array(
		    'title'       => __( 'Adsense Compatible', 'weblog' ),
		    'desc' => __( 'Add Adsense on widgets area.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Header Options', 'weblog' ),
		    'desc' => __( 'Customze the header section as your need.', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Recent Trends', 'weblog' ),
		    'desc' => __( 'You can use the recent trends.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Menu Options', 'weblog' ),
		    'desc' => __( 'Lots of menu options are available to manage the menu section. Appearance > Menus', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Sticky Menu Options', 'weblog' ),
		    'desc' => __( 'Enable Sticky menu on your site.', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes'
	    ),
	    array(
		    'title'       => __( 'Featured Selection', 'weblog' ),
		    'desc' => __( 'This is slider section. Check every options there','weblog' ),
		    'free'  => __('Category','weblog'),
		    'pro'   => __('Post, Category, Tags, And Custom','weblog'),
	    ),
	    array(
		    'title'       => __( 'Featured Section Options', 'weblog' ),
		    'desc' => __( 'Options to manage the sliders', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Featured Carousel Slider', 'weblog' ),
		    'desc' => __( 'Options to manage the carousel slider', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => __('Available with multiple options','weblog'),
	    ),

	    array(
		    'title'       => __( 'Sticky Sidebar', 'weblog' ),
		    'desc' => __( 'Enable/Disable sticky sidebars', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Blog/Archive Options', 'weblog' ),
		    'desc' => __( 'Various Options are available for the blog and archive page management. Check on Appearance > Customize > Layout/Design Options.', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Manage Every Entity','weblog'),
	    ),
	    array(
		    'title'       => __( 'Footer Options', 'weblog' ),
		    'desc' => __( 'Manage the every entity related to footer section.', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Social Options', 'weblog' ),
		    'desc' => __( 'Options to manage the social icons like facebook, instagram, linkedin etc.', 'weblog' ),
		    'free'  => __('Limited','weblog'),
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Theme Credit Link', 'weblog' ),
		    'desc' => __( 'Options to disable to Powered by text on footer.', 'weblog' ),
		    'free'  => __('No Options To Disable','weblog'),
		    'pro'   => __('Enable/Disable','weblog'),
	    ),
	    array(
		    'title'       => __( 'Own Credit Link', 'weblog' ),
		    'desc' => __( 'Write your own credit text.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => __('Your Own Credit Text','weblog'),
	    ),
	    array(
		    'title'       => __( 'Social Icons', 'weblog' ),
		    'desc' => __( 'Enter the URL for social icons. ', 'weblog' ),
		    'free'  => __('3 +','weblog'),
		    'pro'   => __('Available Mostly Used','weblog'),
	    ),
	    array(
		    'title'       => __( 'Intro Loader Options', 'weblog' ),
		    'desc' => __( 'It will load before the site loaded completely.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Related Post Options', 'weblog' ),
		    'desc' => __( 'Related post will appear on single post of each blog article', 'weblog' ),
		    'free'   => 'no',
		    'pro'   => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'       => __( 'Author Info Options', 'weblog' ),
		    'desc' => __( 'This will appear on single page and archive page of that author.', 'weblog' ),
		    'free'   => 'no',
		    'pro'   => 'yes'
	    ),
	    array(
		    'title'       => __( 'Feature Image Options', 'weblog' ),
		    'desc' => __( 'Featured image options for the single page and post.', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => __('Manage As Your Need','weblog'),
	    ),
	    array(
		    'title'       => __( 'Home Page Layout', 'weblog' ),
		    'desc' => __( 'Change your home page layout with available options', 'weblog' ),
		    'free'  => __('Masonry','weblog'),
		    'pro'   => __('Masonry and Normal','weblog'),
	    ),
	    array(
		    'title'       => __( 'Navigation Options', 'weblog' ),
		    'desc' => __( 'Post Navigation means, previous post and next post. Check on single post.', 'weblog' ),
		    'free'   => __('Default','weblog'),
		    'pro'   => __('Advanced','weblog'),
	    ),
	    array(
		    'title'       => __( 'Post Comment Options', 'weblog' ),
		    'desc' => __( 'Manage the comment section on single post.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => __('Options Available','weblog'),
	    ),
	    array(
		    'title'       => __( 'Show/Hide Comment', 'weblog' ),
		    'desc' => __( 'Options to show hide comments on page and post.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Breadcrumb', 'weblog' ),
		    'desc' => __( 'Advanced options for manage the Breadcrumb.', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => __('Advanced','weblog'),
	    ),
	    array(
		    'title'       => __( 'Video Post On Featured', 'weblog' ),
		    'desc' => __( 'Create your video post easily from Youtube and Vimeo video.', 'weblog' ),
		    'free'  => 'no',
		    'pro'   => __('Add Own Video On Featured','weblog'),
	    ),
	    array(
		    'title'=> __( 'Single Post Options', 'weblog' ),
		    'desc' => __( 'Manage every entity of single post.', 'weblog' ),
		    'free' => __('Limited','weblog'),
		    'pro'  => __('Multiple Options','weblog'),
	    ),
	    array(
		    'title'=> __( 'Featured Image Size', 'weblog' ),
		    'desc' => __( 'Select the size of the featured image on the single post.', 'weblog' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Image Size Options', 'weblog' ),
		    'desc' => __( 'Make the image size as your need on blog/archive page.', 'weblog' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Sidebars', 'weblog' ),
		    'desc' => __( 'Global and Individual sidebars are available. You can easily orveride it from single page/post.', 'weblog' ),
		    'free' => 'yes',
		    'pro'  => __('Global And Individuals', 'weblog'),
	    ),
	    array(
		    'title'=> __( 'Infinite Scroll Features', 'weblog' ),
		    'desc' => __( 'Is theme have default infinite scroll ?', 'weblog' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Infinite Scroll', 'weblog' ),
		    'desc' => __( 'Use Jetpack Infinite Scroll on Blog Page', 'weblog' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'WooCommerce', 'weblog' ),
		    'desc' => __( 'Create WooCommerce shop easily with the help of WooCommerce plugin.', 'weblog' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'       => __( 'Translation', 'weblog' ),
		    'desc' => __( 'Both theme are translation ready.', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'SEO', 'weblog' ),
		    'desc' => __( 'Developed with high skilled SEO tools.', 'weblog' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Widget Area On Footer', 'weblog' ),
		    'desc' => __( 'Change the number of footers on footer area as you need.', 'weblog' ),
		    'free'  => __('3 ', 'weblog'),
		    'pro'   => __('Choose On Need', 'weblog'),
	    ),
	    array(
		    'title'       => __( 'Support Forum', 'weblog' ),
		    'desc' => __( 'Highly dedicated support team are assigned for your help. Try this today.', 'weblog' ),
		    'free'  => __('Second', 'weblog'),
		    'pro'   => __('Dedicated With High Priority', 'weblog'),
	    )
    )
);
return new Weblog_Theme_Info( $config );